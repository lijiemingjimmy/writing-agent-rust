...
import { analyzeTeacherJson, askTeacherArchive, deleteTeacherStudentSession, downloadTeacherExport, fetchTeacherClassInsights, fetchTeacherClassSummary, fetchTeacherStats, fetchTeacherStudentDetail, fetchTeacherStudentSummary, fetchTeacherStudents, summarizeTeacher, teacherAccessStorageKey } from "../api";
...
export function TeacherApp() {
  ...
  async function askArchive(question=archiveQuestion){const q=question.trim();if(!q||askingArchive)return;setArchiveQuestion(q);setAskingArchive(true);try{const r=await askTeacherArchive(q,300);setArchiveAnswer(r.answer);setArchiveEvidence(r.evidence||[]);setArchiveError("");}catch(e){setArchiveError(errorText(e));}finally{setAskingArchive(false);}}
